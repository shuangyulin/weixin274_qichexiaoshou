	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussqichexinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussqichexinxi`, this.ruleForm);
				}else{
					await this.$api.add(`discussqichexinxi`, this.ruleForm);
				}
				uni.setStorageSync('discussqichexinxiCleanType',true);
